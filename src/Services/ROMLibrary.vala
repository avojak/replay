...
public class Replay.Services.ROMLibrary : GLib.Object {

    private static Replay.Services.ROMLibrary _instance = null;
    public static Replay.Services.ROMLibrary instance {
        get {
            if (_instance == null) {
                _instance = new Replay.Services.ROMLibrary ();
            }
            return _instance;
        }
    }
    ...
    private void scan_rom_directory (GLib.File rom_directory) {
        ...
        try {
            while ((info = file_enumerator.next_file ()) != null) {
                ...
                found_rom (GLib.File.new_for_path (Constants.LIBRETRO_CORE_DIR + "/" + info.get_name ()));
            }
        } catch (GLib.Error e) {
